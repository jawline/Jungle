function Game(container, windowWidth, windowHeight) {
	...
	scene.fog = new THREE.Fog(0x030303, 10, 20000);
...
function positionVertex(vertex, distance, fixed, deviation) {
...
Game.prototype.testSetup = function() {
	...
	var params = [[new THREE.Vector3(0.0, distance, 0.0), dev], [new THREE.Vector3(0.0, -distance, 0.0), dev], [new THREE.Vector3(0.0, 0.0, distance), dev], [new THREE.Vector3(0.0, 0.0, -distance), dev], [new THREE.Vector3(distance, 0.0, 0.0), dev], [new THREE.Vector3(-distance, 0.0, 0.0), dev]];
	...
	for (var i = 0; i < params.length; i++) {
		...
		for (var j = 0; j < num_particles; j++) {
			...
			positionVertex(vertex, distance * 2, params[i][0], params[i][1]);
	...
	var light1 = new THREE.PointLight(0xffffff, intensity, distance);
